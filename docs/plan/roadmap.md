...
| [M1](milestones/M1.md) | Concise overview, complete-history date picker, trip/charge summaries and charts, responsive navigation; reuse existing PWA foundations | Automated implementation complete: all 36 bounded steps (T01–T36) passed implementation, independent review, and integration checks, with one local commit per step. PostgreSQL-only data in M1; continuous MQTT and advanced analysis excluded. Human acceptance, publication, and deployment remain pending. |
