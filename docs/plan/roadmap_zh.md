...
| [M1](milestones/M1_zh.md) | 精简概览、完整历史日期选择器、行程/充电摘要与曲线、响应式导航；复用已有 PWA 基础 | 已完成：全部 36 个有界步骤（T01–T36）均通过实现、独立审查及集成检查，每步保留一个提交。所有者反馈本地简要走查未发现问题，并确认 M1 交接；这不表示计划中的每项人工场景或真实数据兼容性检查都已逐项执行。M1 仅使用 PostgreSQL 数据，不含持续 MQTT 或高级分析。`v0.1.0-alpha.3` 已在标签检查、原生 amd64/arm64 镜像验证及匿名拉取通过后发布；生产部署与真实数据兼容性验证仍待进行。 |
| M2 | 功能覆盖清单与优先统计：效率、续航/电池、停车掉电、地点 | 提议；M1 后细化 |
| M3 | 根据实际使用增加对比、活动时间线、性能与体验优化 | 提议；范围未承诺 |

发布基线：[`v0.1.0-alpha.3`](https://github.com/t-liu93/matescope/releases/tag/v0.1.0-alpha.3)，镜像 `ghcr.io/t-liu93/matescope:0.1.0-alpha.3`，清单 `sha256:46d92de26e4eb9e645bc8d59c1659a7d349e95f156c59a4e125df4e8f0a464bc`，源提交 `102a63d9fef137787b8c6da87a0882429cbbecc0`。发布完成不代表该版本已部署到 VPS。
