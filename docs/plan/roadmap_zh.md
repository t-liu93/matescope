...
| [M1](milestones/M1_zh.md) | 精简概览、完整历史日期选择器、行程/充电摘要与曲线、响应式导航；复用已有 PWA 基础 | 已完成：全部 36 个有界步骤（T01–T36）均通过实现、独立审查及集成检查，每步保留一个提交。所有者反馈本地简要走查未发现问题，并确认 M1 交接；这不表示计划中的每项人工场景或真实数据兼容性检查都已逐项执行。M1 仅使用 PostgreSQL 数据，不含持续 MQTT 或高级分析。Alpha 发布和生产部署仍待进行。 |
