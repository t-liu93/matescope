...
| [M1](milestones/M1_zh.md) | 精简概览、完整历史日期选择器、行程/充电摘要与曲线、响应式导航；复用已有 PWA 基础 | 已授权按编排流程实现全部 36 个有界步骤：T01–T34 已通过实现、独立审查及集成检查；T35–T36 尚未开始。M1 仅使用 PostgreSQL 数据，不含持续 MQTT 或高级分析。人工验收、发布及部署仍待进行。 |
