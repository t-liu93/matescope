...
已确认方向：自托管、响应式 React PWA，具有独立鉴权、Python/FastAPI 后端、TeslaMate 历史只读访问、可选 MQTT，以及独立可部署容器。所有者通过自己的安装脚本集成部署。先证明真实数据可读，再构建日常 Dashboard。
...
| 里程碑 | 结果 | 状态 |
| --- | --- | --- |
| [M0](milestones/M0_zh.md) | 本地 Compose 预览、网页初始化、只读数据、备份恢复、CI 与双平台发布、VPS 验收 | T01–T07 已实现并通过本地验证；T08 已通过远程 CI 和原生 amd64/arm64 镜像检查，`v0.1.0-alpha.1` 已发布至 GHCR 并验证匿名拉取；T09 VPS 验收待完成 |
| M1 | 日常概览、行程与充电流程、响应式导航及可安装 PWA | 提议；M0 后细化 |
